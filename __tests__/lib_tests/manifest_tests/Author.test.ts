...
describe('Author', () => {
	...
	it('Invalidates undefined argument', () => {
		expect(() => {
			...
			'Validation Error: authorName must be provided as a string, undefined (undefined) received',
		);
	});
	it('Invalidates non-string argument', () => {
		...
		expect(() => {
			...
			'Validation Error: authorName must be provided as a string, 42 (number) received',
		);
		badArgument = {};
		expect(() => {
			new Author(badArgument);
		...
		expect(() => {
			...
			'Validation Error: authorName must be provided as a string, true (boolean) received',
		);
		badArgument = () => {};
		expect(() => {
			new Author(badArgument);
	...
	it('Outputs XML in any context', () => {
		...
		expect(abstract.xml(['manifest.xml'])).toBe(
			'<Author>Extension Creator</Author>\n',
		);
	});
});
