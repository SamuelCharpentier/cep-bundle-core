...
describe('Abstract', () => {
	...
	it('Invalidates undefined argument', () => {
		expect(() => {
			...
			'Validation Error: abstract must be provided as a valid URL (type), undefined (undefined) received',
		);
	});
	it('Invalidates non-URL argument', () => {
		...
		expect(() => {
			...
			'Validation Error: abstract must be provided as a valid URL (type), 42 (number) received',
		);
		badArgument = {};
		expect(() => {
			new Abstract(badArgument);
		...
		expect(() => {
			...
			'Validation Error: abstract must be provided as a valid URL (type), true (boolean) received',
		);
		badArgument = () => {};
		expect(() => {
			new Abstract(badArgument);
	...
	let validArgument: URL | string = new URL(
		'http://myextension.com/abstract.html',
	);
	...
	it('Outputs XML in any context', () => {
		...
		expect(abstract.xml(['manifest.xml'])).toBe(
			'<Abstract href="http://myextension.com/abstract.html"/>\n',
		);
		validArgument =
			'http://myextension.com/some/weird/sub/url/as/string/abstract.html';
