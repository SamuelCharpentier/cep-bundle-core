...
export function getConfig(pkg: any, env?: string) {
  const config = assignDefined({}, getConfigDefaults(), getPkgConfig(pkg), getPkgConfig(pkg, env), getEnvConfig())
    ...
    .then(() => {
      ...
      if (needsNewLink) {
        ...
        if (process.platform === 'win32') {
          ...
          sudo.exec(
            `node -e "require('fs').symlink('${symlinkTarget.replace(/\\+/g, '\\\\')}', '${symlinkPath.replace(
              /\\+/g,
              '\\\\',
